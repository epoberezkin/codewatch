...
Shown when any of `threatModel`, `threatModelParties`, or `threatModelFileLinks` is present.
...
- ~~[GAP] The threat model text is truncated to 2000 characters in the fallback (no parties) rendering path, with no indication to the user that content was cut off.~~ (RESOLVED: Threat model is now parsed into structured fields and rendered in full without truncation.)
